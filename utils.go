...
func PrintTelegraf(tag *string, ip *string, ip2loc IP2Location, mes *string, tm *string) {
	...
	if tagFieldValue != "" {
		...
		fmt.Printf("%s,host=\"%s\",country_long=\"%s\",country_short=\"%s\",city=\"%s\",state=\"%s\",timezone=\"%s\" country_long=\"%s\",country_short=\"%s\",city=\"%s\",state=\"%s\",timezone=\"%s\",latitude=%f,longitude=%f,packetSent=%d,packetRecv=%d,packetLost=%f,minRtt=%v,avgRtt=%v,maxRtt=%v,online=%d", *mes, *ip, ip2loc.CountryLong, ip2loc.CountryShort, ip2loc.City, ip2loc.Region, ip2loc.Timezone, ip2loc.CountryLong, ip2loc.CountryShort, ip2loc.City, ip2loc.Region, ip2loc.Timezone, ip2loc.Lat, ip2loc.Lon, stats.PacketsSent, stats.PacketsRecv, stats.PacketLoss, minRTT, avgRTT, maxRTT, isHostOnline)
	}
	fmt.Printf("\n")
